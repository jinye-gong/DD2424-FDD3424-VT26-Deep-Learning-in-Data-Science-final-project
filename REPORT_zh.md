...
> **目标成绩：A/B**（见下表与各节 **[E] / [D/C] / [B/A]** 标注）

---

## 等级任务对照（Default Project 2）

课程将项目分为 **E → D/C → B/A** 三档（见 `DefaultProject2.pdf` §1.1–1.2）。下表标明本组对应工作与报告章节；正文实验节均带等级标签。

| 等级 | 课程要求（摘要） | 本组完成情况 | 报告位置 |
|------|------------------|--------------|----------|
| **[E]** | Assignment 3 改写为 PyTorch；堆叠 VGG block；AdamW；dropout / weight decay / 数据增强 / BN；探索学习率调度；CIFAR-10 达到较高 test 精度 | `vgg_baseline`：BN + Dropout 0.5 + RandomCrop/Flip + AdamW + cosine+warmup；**test 92.78%**（w=64，200 epoch） | §3.2，§4.1，§5.1 |
| **[D/C]** | 在 E 基础上至少完成 **2 项**扩展（现代归一化 / 增强 / 训练配方 / 架构改进 / 评估指标） | **(1) 现代训练配方**：warmup + cosine、AdamW、weight decay（`configs/default.yaml`）<br>**(2) 架构改进**：width 缩放（WideResNet 思路）；另在 CN 块中使用 LayerNorm（现代归一化组件） | §3.2，§5.4 |
| **[B/A]** | 实质性扩展：清晰 RQ、对照实验与消融；公平比较现代架构与强 VGG；报告主体写扩展 | **ConvNet vs 现代块** + **现代组件**：VGG / DW / CN 在匹配 FLOPs 下对比；width、kernel、depth 扫描；Pareto 与设计规则（proposal RQ1–RQ2） | §1.2，§4.3–4.5，§5.3–5.7，§6 |

**[D/C] 所选方向明细（对应作业 bullet）：**

| 方向 | 是否采用 | 具体实现 |
|------|----------|----------|
| 现代归一化（GroupNorm / LayerNorm） | 部分 | ConvNeXt 块内 `LayerNorm2d`；VGG/DW 仍用 BN |
| 改进数据增强（MixUp / CutMix 等） | 未作主要贡献 | 统一配方中 **MixUp/AMP 关闭**（消融约定） |
| **现代训练配方** | **是** | AdamW + 5 epoch warmup + cosine；weight decay 0.05 |
| **架构改进** | **是** | 深度可分离块、ConvNeXt 块；**width 缩放**；三档 FLOPs 对齐 |
| 改进评估指标（校准 / 鲁棒性） | 否 | 主指标：test/val accuracy、FLOPs、Pareto |

**[B/A] 扩展任务明细（对应 proposal + 作业 §1.2.2）：**

| 任务 | 状态 | 实验 / 结果 |
|------|------|-------------|
| 公平比较 VGG vs 现代块（控制 FLOPs） | **9/9** | 三档位 × 三 block，表 4 |
| 现代组件：大核、LayerNorm、倒瓶颈 | 已实现 + 消融 | CN 族；kernel 扫描表 7 |
| width / kernel / depth 缩放律 | **17/17** | §5.4–5.7；图 3–6 |
| Pareto 与 budget-aware 规则 | 完成 | §5.5，§6.4 |
| 明确 RQ 与结论 | 完成 | RQ1 否；RQ2 width/DW med 最优；大核负收益 |
...
第 2 节相关工作；第 3 节方法；第 4 节实验设计（含 **[E]/[D/C]/[B/A]** 标注）；第 5 节结果；第 6–7 节讨论与结论；第 8 节分工；附录复现说明。
...
#### 3.3.1 VGG 风格基线 — `vgg_baseline` **[E]**

Conv–BN–ReLU 堆叠，三次 MaxPool，MLP 分类头；可调 `width`。

#### 3.3.2 深度可分离 — `depthwise_small` **[B/A]**

DW + 1×1 PW + BN + ReLU；stage 布局与 VGG 对齐；可调 `width`、`kernel_size`。

#### 3.3.3 ConvNeXt 风格 — `convnext_small` **[B/A]**（含 **[D/C]** LayerNorm）
...
本组所有实验在**统一训练底座**上顺序执行，保证可比性。

**阶段 0 — 环境与复现约定**

1. 克隆仓库，安装 `requirements.txt`，激活 conda 环境 `dd2424`。
2. 固定验证集索引 `data/artifacts/cifar10_val_indices.pt`（`seed=42`，`val_ratio=0.1`）。
3. 全组共用 `configs/default.yaml`；FLOPs 统一用 `utils/metrics.count_flops()`（FLOPs = 2×MACs）。

**阶段 1 — [E] 强基线（1 次主 run + 2 次 profile）**

| 步骤 | 命令 / 配置 | 目的 |
|------|-------------|------|
| 1 | `python scripts/train.py --model vgg_baseline --width 64` | 200 epoch 完整训练 |
| 2 | 记录 train/val 曲线；按 **val 最高** 存 checkpoint | 模型选择仅用 val |
| 3 | 训练结束后 **一次** test 评估 | 得 test **92.78%** |
| 4 | `profile_model.py` 统计三族默认 w=64 的 Params/FLOPs | 发现未对齐（表 2） |

**阶段 2 — [B/A] FLOPs 档位对齐（9 runs）**

1. 在 `configs/experiments.yaml` 中为 low/med/high 三档预选 `width`（VGG/DW/CN 各一）。
2. `python scripts/run_experiments.py` 顺序训练，每 run 200 epoch，`seed=42`。
3. 每 run 结束写入 `logs/results.csv`；**不**在 test 上调参。

**阶段 3 — [B/A] Kernel 扫描（7 runs，含 2 个 tier 默认点）**

- 固定 CN `width=52`、DW `width=175`（med 预算附近）。
- 扫描 `kernel_size ∈ {3,5,7,11}`；CN 默认块内 k=7 计入 k=7 行；DW 默认 k=3 计入 k=3 行。

**阶段 4 — [B/A] Depth 扫描（2 runs）**

- 浅层 `depths=(1,1,1), width=64`：首次因中断仅 2 epoch（test 41.67%），**2026-05-23 补训** 至 200 epoch（test 89.16%）。
- 深层 `depths=(3,3,3), width=43`：与 med 档 FLOPs 接近，用于对比“加深 vs 减宽”。

**阶段 5 — 汇总与作图**

```bash
python scripts/generate_report_assets.py   # 读 results.csv，生成 figures/ 与 REPORT_DATA_zh.md
```

**完成度（2026-05-23）：** 计划队列 **17/17** 均已训满 200 epoch。

---

### 4.1 基线复现 **[E]**

`vgg_baseline` w=64，200 epoch，记录 train/val 曲线，最佳 val checkpoint 做一次 test 评估。对应 Default Project §1.1（E 级：VGG + 正则 + AdamW + 调度 + 高精度）。

**过程要点：** 约 100 epoch 后 val 趋于饱和；train acc 接近 100%，存在轻度过拟合；best val 93.92% 出现在 epoch 197 附近，test 仍达 92.78%，满足 E 级「high eighties」以上目标。
...
### 4.3 Block 类型对比（匹配 FLOPs）**[B/A]**

低 / 中 / 高三档，每档 VGG、DW、CN 各一配置（`configs/experiments.yaml`）。对应作业「ConvNet vs modern architectures」+ proposal 公平 FLOPs 对比。

### 4.4 缩放实验 **[B/A]**（含 **[D/C]** 架构 width）

Width、kernel、depth 扫描均已 **17/17** 完成（200 epoch）。Width 同时满足 D/C「架构改进 / WideResNet 式缩放」。

### 4.5 Pareto 与边际收益 **[B/A]**
...
| # | 发现 | 证据 |
|---|------|------|
| F1 | **全局最优**为 VGG w=64：309.5M FLOPs，**test 92.78%** | 表 3、表 6、图 4 |
| F2 | **三档 FLOPs 对齐后 VGG test 均为第一** | 表 4、图 2 |
| F3 | Med 档 **DW** 用约 90% FLOPs 达到 VGG **99.4%** 精度（92.18% vs 92.78%） | 表 4 |
| F4 | **CN 在三档均最低**；high 档 462M FLOPs 仍仅 90.12%，低于 med 档 VGG | 表 4 |
| F5 | VGG **加宽至 w=80 反而降点**（92.46% < 92.78%） | 表 4、表 5 |
| F6 | **Kernel 增大对 CN/DW 均为负收益**，test 随 k 单调降 | 表 7、图 5 |
| F7 | k=11 时 FLOPs 最高、精度最低（CN 88.21%，DW 88.49%） | 表 7 |
| F8 | CN **加深 (3,3,3) 略优于减宽 (1,1,1)**，但均远低于 VGG/DW | §5.7 |
| F9 | 未对齐默认 width 不能直接回答 RQ1（DW 仅 40.4M FLOPs） | 表 2 |
...
### 5.3 匹配 FLOPs 后的 Block 对比（表 4）**[B/A]**
...
本研究按 **E → D/C 训练配方 → B/A 扩展实验** 推进：先建立 92.78% test 的 VGG 强基线，再在统一 pipeline 上完成 17 次 200-epoch 训练（档位对齐 9 + kernel 7 + depth 2），全程固定 val 划分与超参，由 `run_experiments.py` 顺序执行并汇总至 `results.csv`。

### 7.2 实验发现（对应 RQ）

**RQ1 — ConvNeXt 是否在匹配 FLOPs 下优于 VGG？**

**否。** 三档对齐中 CN 分别低 **3.20 / 2.44 / 2.34** test 点；CN 最佳配置（k=3，91.85%）仍低于 med 档 VGG（92.78%）。在相同算力预算下，**现代 ConvNeXt 块未带来更优 accuracy–FLOPs 权衡**。

**RQ2 — 哪一缩放方向边际收益最高？**

| 方向 | 结论 |
|------|------|
| **Block 类型** | VGG > DW > CN（全档位） |
| **Width** | VGG 在 w=64 达峰；DW 在 low→med 收益最大（+1.04 pt/100M FLOPs）；继续加宽无益 |
| **Kernel** | **负收益**：k=3 最优，k=11 降约 3.7 pt |
| **Depth（CN）** | (3,3,3) > (1,1,1)，但均无法追上 VGG/DW |

### 7.3 实验结论（定稿）

1. **Pareto 最优族为 VGG 标准卷积**；全局最高点 `vgg_baseline_w64`（92.78% @ 309.5M）。
2. **算力受限时首选深度可分离 + k=3**（med 档 92.18% @ 280M，效率最佳）。
3. **不宜在 32×32 上照搬 ImageNet 式大核 ConvNeXt**；增大 kernel 与堆高 FLOPs 均不能弥补精度差距。
4. **更多 FLOPs 不等于更高精度**（VGG w=80、high 档 CN 均为反例）。

**实践建议：** CIFAR-10 优先 **VGG w=48/64**；FLOPs 敏感用 **DW w=175, k=3**；CN 仅作低优先级备选，且避免 k≥7。

### 7.4 后续工作

多种子与置信区间；GPU 延迟与能耗；更高分辨率迁移验证。
